...
/**
 * The type Yaw compensation example.
 */
public class YawCompensationExample {
    ...
    public static void main (final String[] args) throws Exception {
        ...
        final Header header = new Header("Yaw compensation with 3 satellites with the same fov", clock, pathToJSFolder);
        ...
        final KeplerianOrbit secondOrbit = new KeplerianOrbit(7200000, 0, FastMath.toRadians(98), 0,
                FastMath.toRadians(20), FastMath.toRadians(0), PositionAngleType.MEAN, FramesFactory.getEME2000(), startDate,
                Constants.WGS84_EARTH_MU);
        final KeplerianOrbit thirdOrbit = new KeplerianOrbit(7200000, 0, FastMath.toRadians(98), 0,
                FastMath.toRadians(0), FastMath.toRadians(40), PositionAngleType.MEAN, FramesFactory.getEME2000(), startDate,
                Constants.WGS84_EARTH_MU);
        ...
        final AdaptiveStepsizeIntegrator integrator2 = new DormandPrince853Integrator(TutorialUtils.MIN_STEP,
                ...
                tolerances2[1]);
        final AdaptiveStepsizeIntegrator integrator3 = new DormandPrince853Integrator(TutorialUtils.MIN_STEP,
                TutorialUtils.MAX_STEP, tolerances3[0],
                tolerances3[1]);
        ...
        final NadirPointing nadirPointing = new NadirPointing(FramesFactory.getEME2000(), TutorialUtils.getEarth());

        final YawCompensation yawCompensation = new YawCompensation(FramesFactory.getEME2000(), nadirPointing);

        final YawSteering yawSteering = new YawSteering(FramesFactory.getEME2000(), nadirPointing, sunModel,
                Vector3D.PLUS_J);
        ...
        final FieldOfObservation fieldOfObservation1 = FieldOfObservation.builder(satellite1, fov1, initialFovBody1, header)
        ...
        final FieldOfView fov2 = new DoubleDihedraFieldOfView(Vector3D.PLUS_K, Vector3D.PLUS_I, FastMath.toRadians(20),
                Vector3D.PLUS_J, FastMath.toRadians(5), 2);
        final FieldOfObservation fieldOfObservation2 = FieldOfObservation.builder(satellite2, fov2, initialFovBody2, header)
                                                                         .withColor(Color.GREEN)
                                                                         .build();
        ...
        final FieldOfView fov3 = new DoubleDihedraFieldOfView(Vector3D.PLUS_K, Vector3D.PLUS_I, FastMath.toRadians(20),
                Vector3D.PLUS_J, FastMath.toRadians(5), 2);
        final FieldOfObservation fieldOfObservation3 = FieldOfObservation.builder(satellite3, fov3, initialFovBody3, header)
                                                                         .withColor(Color.GREEN)
                                                                         .build();
