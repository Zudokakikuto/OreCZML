...
public class FieldOfObservation
    ...
    /**
     * The list of reference that defines the point on the ground that will move
     * in time.
     */
     ...
     */
    private List<List<List<GeodeticPoint>>> footprintsInTime =
        new ArrayList<>();

    /**
     * The list of all the points on body that represents the limits of the
     * field of view projected on the body.
     */
    private final List<PointOnBody> points = new ArrayList<>();

    /**
     * A boolean that trigger if at least the fov is projected one time to the
     * body.
     */
     ...
     * @param angularStepInput : The angular step between two footprints on the
     *        body (projection of points on the body).
    ...
    FieldOfObservation(final Spacecraft spacecraft, final FieldOfView fovInput,
        ...
        for (int i = 0; i < julianDates.size(); i++) {
            ...
            final Transform currentInertToBody =
            ...
            final List<List<GeodeticPoint>> currentFootprints =
                fovInput.getFootprint(currentTransformFovToBody, body,
                                      angularStepInput);
            footprintsInTime.add(currentFootprints);
        }
        if (footprintsInTime.get(0).isEmpty()) {
            // Could not display the field of view, it does not cross the
    ...
    public void writeCzmlBlock(final CesiumStreamWriter stream,
        ...
        if (!noDetection) {
            ...
            for (int i = 0; i < footprintsInTime.size(); i++) {
                ...
                buildPolyline(polylineColor, referenceSatellite,
                              secondReference, true, stream, output);
            }
            // Display outlines of the line of sight (except the last one
            // because of the -1)
            for (int i = 0; i < points.size() - 1; i++) {
                final PointOnBody currentPoint = points.get(i);
                final PointOnBody nextPoint = points.get(i + 1);
                final Reference currentPointReference =
                    new Reference(currentPoint.getId() + DEFAULT_H_POSITION);
                final Reference secondPointReference =
                    new Reference(nextPoint.getId() + DEFAULT_H_POSITION);
                buildPolyline(polylineColor, currentPointReference,
                              secondPointReference, false, stream, output);
            }
            ...
            final Reference lastPointReference =
                new Reference(lastPoint.getId() + DEFAULT_H_POSITION);
            final Reference firstPointReference =
    ...
    /**
     * This functions aims at extracting the cartesian coordinates from the
     * geodetic coordinates.
     *
     * @param geodeticPoints : A list of list of list of geodetic points that
     *        represents the footprints of the field of view on the body in
     *        time. Each list is attributed to a specific projected point, then
     *        the sublist represents all the points on the body in time for this
     *        given point.
     * @return : A list of list of list of cartesian that represents the
     *         positions of the projected point on the body in time. Each list
     *         is attributed to a specific projected point, then each sublist
     *         represents all the points on the body in time for this given
     *         point.
     */
    private List<List<List<Cartesian>>>
        extractCartesianFromGeodetic(final List<List<List<GeodeticPoint>>> geodeticPoints) {
        ...
        for (List<List<GeodeticPoint>> geodeticPoint : geodeticPoints) {
            ...
            for (int j = 0; j < geodeticPoints.get(0).size(); j++) {
                ...
                for (int k = 0; k < geodeticPoint.get(j).size(); k++) {

                    final GeodeticPoint currentGeodeticPoint =
                        geodeticPoint.get(j).get(k);
                    final TopocentricFrame currentTopocentricFrame =
                        new TopocentricFrame(body, currentGeodeticPoint, "");
                    final Vector3D currentVector3D =
                        currentTopocentricFrame.getCartesianPoint();
                    final Cartesian currentCartesian =
                        new Cartesian(currentVector3D.getX(),
     ...
     * @param firstPointReference : The first point of reference for the
     *        extremity of the polyline
     * @param secondPointReference : The second point of reference for the
     *        extremity of the polyline
     * @param line : To know if the polyline is a line linked with the
     *        satellite, or a line linking two points on the ground
     ...
     */
    private void buildPolyline(final Color polylineColorInput,
                               final Reference firstPointReference,
                               final Reference secondPointReference,
                               final boolean line,
        ...
        try (PacketCesiumWriter packet = stream.openPacket(output)) {
            ...
            if (line) {
                packet.writeName("Line of the Line of observation : " +
                                 numberOfPolylines);
            } else {
                packet.writeName("Outline of the line of observation : " +
                                 numberOfPolylines);
            }
     ...
     */
    private <T> List<List<List<T>>>
        sortingListListList(final List<List<List<T>>> objects) {
        ...
        for (int i = 0; i < objects.get(0).get(0).size(); i++) {
            ...
            for (int j = 0; j < objects.get(0).size(); j++) {
                ...
                for (List<List<T>> object : objects) {
                    final T objectToSort = object.get(j).get(i);
