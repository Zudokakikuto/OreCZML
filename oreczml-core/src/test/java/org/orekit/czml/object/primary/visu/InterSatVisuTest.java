...
public class InterSatVisuTest
    ...
    void InterSatVisuConstructorTest()
        ...
        final KeplerianOrbit firstOrbit =
                        new KeplerianOrbit(7878000, 0, FastMath.toRadians(0), FastMath.toRadians(0), FastMath.toRadians(0), FastMath.toRadians(0),
        ...
        final KeplerianOrbit secondOrbit =
                        new KeplerianOrbit(7078000, 0, FastMath.toRadians(20), FastMath.toRadians(45), FastMath.toRadians(0), FastMath.toRadians(0),
                                           PositionAngleType.MEAN, FramesFactory.getEME2000(), startDate, Constants.WGS84_EARTH_MU);
        final SpacecraftState secondState = new SpacecraftState(secondOrbit);

        // Build of the propagator

        final double[][] tolerances1 = NumericalPropagator.tolerances(10, firstOrbit, OrbitType.CARTESIAN);

        final double[][] tolerances2 = NumericalPropagator.tolerances(10, secondOrbit, OrbitType.CARTESIAN);

        final AdaptiveStepsizeIntegrator integrator1 = new DormandPrince853Integrator(0.001, 1000.0, tolerances1[0], tolerances1[1]);

        final AdaptiveStepsizeIntegrator integrator2 = new DormandPrince853Integrator(0.001, 1000.0, tolerances2[0], tolerances2[1]);

        final NormalizedSphericalHarmonicsProvider provider = GravityFieldFactory.getNormalizedProvider(10, 10);
        final ForceModel holmesFeatherstone = new HolmesFeatherstoneAttractionModel(FramesFactory.getEME2000(), provider);
        ...
        final Spacecraft firstSat = new Spacecraft(firstBoundedPropagator, header.getClock());
        final Spacecraft secondSat = new Spacecraft(secondBoundedPropagator, header.getClock());
        ...
        final InterSatVisu interSatVisu = InterSatVisu.builder(firstSat, secondSat, finalDate, header.getClock()).build();

        firstBoundedPropagator.clearEventsDetectors();
        firstBoundedPropagator.clearStepHandlers();

        final InterSatVisu interSatVisuBuilder = InterSatVisu.builder(firstSat, secondSat, finalDate, header.getClock()).build();

        firstBoundedPropagator.clearEventsDetectors();
        firstBoundedPropagator.clearStepHandlers();

        final InterSatVisu interSatVisuPropagators = InterSatVisu.builder(propagators, finalDate, header.getClock()).build();
        ...
        final InterSatVisu interSatVisuPropagatorsBuilder = InterSatVisu.builder(propagators, finalDate, header.getClock()).build();
        ...
        final InterSatVisu interSatVisuPropagatorsID =
                        InterSatVisu.builder(propagators, finalDate, header.getClock()).withCustomId("CustomIDTest").build();
        ...
        final InterSatVisu interSatVisuFromConstellationBuilder =
                        InterSatVisu.builder(Constellation.builder(propagators, finalDate, header.getClock()).build(), finalDate, header.getClock())
                                        .build();
        ...
        final InterSatVisu interSatVisuFromConstructor = new InterSatVisu(firstSat, secondSat, finalDate);
        firstBoundedPropagator.clearEventsDetectors();
        firstBoundedPropagator.clearStepHandlers();
        secondBoundedPropagator.clearStepHandlers();
        secondBoundedPropagator.clearEventsDetectors();

        final InterSatVisu interSatVisuFromPropagators = new InterSatVisu(propagators, finalDate, clock);
        firstBoundedPropagator.clearEventsDetectors();
        firstBoundedPropagator.clearStepHandlers();
        secondBoundedPropagator.clearStepHandlers();
        secondBoundedPropagator.clearEventsDetectors();

        final Constellation constellation = Constellation.builder(propagators, finalDate, clock).build();
        final InterSatVisu interSatVisuFromConstellation = new InterSatVisu(constellation, finalDate, header.getAvailability());

        final String pathFile = loadResources("templateFile/object/primary/visu/InterSatVisuTemplate.txt");
        final String propagatorsInterSatPathFile = loadResources("templateFile/object/primary/visu/InterSatVisuPropagatorsTemplate.txt");
        final String propagatorsIDInterSatPathFile = loadResources("templateFile/object/primary/visu/InterSatVisuPropagatorsIDTemplate.txt");
        final String constellationInterSatPathFile = loadResources("templateFile/object/primary/visu/InterSatVisuConstellationTemplate.txt");
        final String constructorInterSatPathFile = loadResources("templateFile/object/primary/visu/InterSatVisuFromConstructorTemplate.txt");
        ...
        verifyFileOutput(constructorInterSatPathFile, interSatVisuFromConstructor.toString(), 1e-8);
        verifyFileOutput(constructorPropagatorsInterSatPathFile, interSatVisuFromPropagators.toString(), 1e-8);
        verifyFileOutput(constructorConstellationInterSatPathFile, interSatVisuFromConstellation.toString(), 1e-8);

        // Getters coverage
        Assertions.assertEquals(secondSat.getSpaceCraftStates().get(0).getPVCoordinates().toString(),
                                interSatVisu.getSatellite2().getSpaceCraftStates().get(0).getPVCoordinates().toString());
        Assertions.assertEquals(startDate, interSatVisu.getStartDate());
        Assertions.assertEquals(finalDate, interSatVisu.getStopDate());
        Assertions.assertEquals(firstSat.getSpaceCraftStates().get(0).getPVCoordinates().toString(),
                                interSatVisu.getInitialState().getPVCoordinates().toString());
        ...
        Assertions.assertEquals(Polyline.nonVectorBuilder(header.getAvailability()).build().getAvailability(),
                                interSatVisu.getPolyline().getAvailability());
        Assertions.assertEquals(new ArrayList<>(Arrays.asList(true, false, true, false, true, false, true)), interSatVisu.getBooleanList());
