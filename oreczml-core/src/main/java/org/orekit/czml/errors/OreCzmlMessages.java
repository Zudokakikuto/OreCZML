 ...
 * OreCzmlMessages.
 * <p>
 * This class contains all the error messages that can happens when the execution of the code is not nominal.=
 *
...
public enum OreCzmlMessages
    ...
    /**
     * Trigger when a primary object is not valid and lacks some of its parameters.
     */
    ...
    // Czml ground station
    /**
     * Triggers when a single station is defined but the user tried to get multiple stations.
     */
    ...
    // Czml File
    /**
     * Triggers when no object (except the header) has been defined in the czml file.
     */
    ...
    // Ground track
    /**
     * Trigger when the getAllGroundTracks() method is call and the ground tracks are empty, it can happen when the czml file has already been written
    ...
    // Orientation
    /**
     * Triggers when several attitudes are defined, but the user uses the getSingleAttitude() method.
     */
    ...
    /**
     * Triggers when a single attitude is defined, but the user uses the getAttitudes() method.
     */
    ...
    /**
     * Triggers when the user input more than 2 cartesians to build a vector polyline that takes two cartesian coordinates as an input. (each
     * coordinate represents the extremity of the line).
     */
    ...
    /**
     * Triggers when the position type given is not in the position type defined. (CARTESIAN_POSITION, CARTESIAN_VELOCITY, CARTOGRAPHIC_RADIANS,
    ...
    /**
     * Trigger when the list of julian dates and the list of cartesian is not the same while trying to build segregated time and position for each
     * influence sphere crossing.
     */
    NOT_SAME_SIZE_TIME_POSITION(
                    "The list of julian dates and the list of cartesian is not the same, thus the influence sphere crossing segregation cannot happen"),
