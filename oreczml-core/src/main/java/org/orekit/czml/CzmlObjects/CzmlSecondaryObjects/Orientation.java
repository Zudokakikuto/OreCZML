...
import org.orekit.czml.ArchiObjects.Builders.OrientationBuilder;
...
public class Orientation implements CzmlSecondaryObject {
    ...
    public Attitude getSingleAttitude() {
        if (singleAttitude == null) {
            throw new RuntimeException(OreCzmlExceptions.MULTIPLE_ATTITUDES_SINGLE_GET);
        } else {
    ...
    public List<Attitude> getAttitudes() {
        if (attitudes.isEmpty()) {
            throw new RuntimeException(OreCzmlExceptions.SINGLE_ATTITUDE_MULTIPLE_GET);
        } else {
            return attitudes;
        }
    }
