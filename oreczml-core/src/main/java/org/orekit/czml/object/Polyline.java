...
public class Polyline {
     ...
     */
    public static NonVectorPolylineBuilder nonVectorBuilder(final Header header) {
     ...
     */
    public static VectorPolylineBuilder vectorBuilder(final List<Cartesian> cartesians, final Header header) {
        ...
        try (PacketCesiumWriter packet = streamWriter.openPacket(output)) {
            if (arrow) {
                ...
                if (firstReference != null) {
                    ...
                    final Iterable<Reference> iterableReferences = convertToIterable(references);
                    this.writePolylineOfVisibility(packet, output, iterableReferences,
                            new ArrayList<>(List.of(new CzmlShow(show, availability))));
                } else {
                    try (PolylineCesiumWriter polylineCesiumWriter = packet.getPolylineWriter()) {
                        polylineCesiumWriter.open(output);
                        this.writePositionOfVectorInertial(polylineCesiumWriter, output);
                    }
                }
            }
        }
     ...
     * @param output               : The cesium output stream that writes inside the czml file.
     */
