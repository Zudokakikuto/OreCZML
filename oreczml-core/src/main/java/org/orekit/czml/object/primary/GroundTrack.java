 ...
 * @since 1.0.0
 */
public class GroundTrack extends AbstractPrimaryObject {
    ...
    public GroundTrack(final Satellite satellite, final BodyShape body, final Color color, final String customID,
        ...
        this.color = color;
        final List<AbsoluteDate> satelliteDates = satellite.getAbsoluteDateList();
        this.states = satellite.getSpaceCraftStates();
        final List<Cartesian> projectedCartesianList = new ArrayList<>();
        for (int i = 0; i < states.size(); i++) {
            final Vector3D projectedVector3D = body.projectToGround(states.get(i)
                                                                          .getPosition(), satelliteDates.get(i),
                    body.getBodyFrame());
            projectedCartesianList.add(
                    new Cartesian(projectedVector3D.getX(), projectedVector3D.getY(), projectedVector3D.getZ()));
        }
        this.clampedPositionOnBody = new TimePosition(projectedCartesianList, satellite.getJulianDates());
    }
            ...
            for (final GroundTrack currentGroundTrack : groundTracks) {
                try (PacketCesiumWriter packet = stream.openPacket(output)) {
                    ...
                    if (displayLinkSatellite) {
                        ...
                        final List<CzmlShow>      shows             = new ArrayList<>();
                        shows.add(show);
                        final Polyline polylineInput = Polyline.nonVectorBuilder(header)
     ...
     * @param output : The output stream of cesium that will contain the strings to write into the CzmLFile.
     */
    private void writeCzmlPath(final PacketCesiumWriter packet, final CesiumOutputStream output) {
