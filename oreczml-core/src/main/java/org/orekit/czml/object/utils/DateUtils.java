...
public class DateUtils {
    ...
    public static double secondsInJulianDate(final AbsoluteDate date) {
        ...
        final int hour =
            date.getComponents(TimeScalesFactory.getUTC()).getTime().getHour();
        final int min =
            date.getComponents(TimeScalesFactory.getUTC()).getTime()
                .getMinute();

        // Number of seconds since the last full minute, including fractional
        // seconds
        final double sec =
            FastMath.floor(date.getComponents(TimeScalesFactory.getUTC())
                .getTime().getSecond());
    ...
    public static JulianDate toJulianDate(final AbsoluteDate date) {
        ...
        // The getJD() function always returns the JD in UTC, so this should be
        // safe.
        // Note: using "round" for now on assumption that output time will
        // always want to be to nearest second.
        final int numDays = (int) FastMath.floor(date.getJD());
