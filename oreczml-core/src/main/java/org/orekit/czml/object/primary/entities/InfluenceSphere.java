...
public class InfluenceSphere extends AbstractPrimaryObject {
    ...
    /**
     * The default constructor of the influence sphere assuming the body is orbiting around the sun.
     * The body object MUST BE WRITTEN in the Czml file for the sphere of influence to work.
     *
     ...
     *
     * @param bodyInput   : The body considered for the sphere of influence.
     * @param centralBody : The central body around which the body is orbiting around.
     * @param headerInput : The header considered.
     */
    InfluenceSphere(final Body bodyInput, final Body centralBody, final Header headerInput) {
        final AbsoluteDate startDate = DateUtils.toAbsoluteDate(headerInput.getAvailability()
                                                                           .getStart(), headerInput.getTimeScale());

        final double centralBodyMass = centralBody.getCelestialBody()
                                                  .getGM() / GRAVITATIONAL_CONSTANT;
        final Frame centralFrame = centralBody.getCelestialBody()
                                              .getInertiallyOrientedFrame();

        this.header          = headerInput;
        this.body            = bodyInput;
        this.centralBodyMass = centralBodyMass;
        this.bodyMass        = bodyInput.getCelestialBody()
                                        .getGM() / GRAVITATIONAL_CONSTANT;
        ...
        final PVCoordinates initialPVCBody = body.getCelestialBody()
                                                 .getPVCoordinates(startDate, centralFrame);
        final KeplerianOrbit orbit = new KeplerianOrbit(initialPVCBody, centralFrame, startDate,
                centralBodyMass * GRAVITATIONAL_CONSTANT);

        final double semiMajorAxis = orbit.getA();
        this.radius = semiMajorAxis * FastMath.pow((bodyMass / centralBodyMass), 0.4);
        final Cartesian cartesianForSphericalEllipsoid = new Cartesian(radius, radius, radius);

        this.cartesianPosition = bodyInput.getCartesianPositionList();
        this.ellipsoid         = CzmlEllipsoid.builder(cartesianForSphericalEllipsoid, header)
                                              .withFill(false)
                                              .withSliceStackPartition(10, 10)
                                              .withOutline(true)
                                              .build();
        this.positionReference = new Reference(bodyInput.getId() + DEFAULT_H_POSITION);
    }

    /** This builder assumes the body is orbiting around the sun. */
    public static InfluenceSphereBuilder builder(final Body bodyInput, final Header headerInput) {
    ...
    public static InfluenceSphereBuilder builder(final Body bodyInput, final Body centralBodyInput,
                                                 final Header headerInput) {
        return new InfluenceSphereBuilder(bodyInput, centralBodyInput, headerInput);
    }
